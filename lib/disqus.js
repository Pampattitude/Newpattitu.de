...
exports.listPosts = function(callback) {
    ...
    var cacheData = memoryCache.get('disqus-post-list');
    if (cacheData)
        return callback(null, cacheData);

    return exports.client.forums.listPosts({
        forum: 'pampattitude',
    }, function (err, res) {
        if (err)
            return callback(err);

        memoryCache.put('disqus-post-list', res.response, constants.disqusDataCacheTime);
        return callback(null, res.response);
    });
};
