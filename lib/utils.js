...
exports.bbCodeToHtml = function(str) {
    ...
    ret = ret
        ...
        .replace(/\[inline-code(=([^\]]*))?\]((?:(?!\[\/inline-code\])[^])+)\[\/inline-code\]/gi, function(oldText, unused1, language, text) {
            if (language) { // Includes language
                console.log(htmlEntities.decode(text));
                try { return '<pre class="inline-code code-' + language + '">' + highlight(htmlEntities.decode(text), language) + '</pre>'; }
                catch (err) { printer.error(err); return oldText; }
            }
            return '<pre class="inline-code">' + text + '</pre>';
        }) // Inline code
        ...
        .replace(/\[unity\]((?:(?!\[\/?unity\]).)+)\[\/unity\]/gi, '<div data-unity3d-player="$1"></div>') // Unity3D Web Player, arg is URL
    ;
